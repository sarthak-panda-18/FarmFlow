...
class _AddRequirementScreenState extends State<AddRequirementScreen> {
  ...
  Future<void> _loadMarkets(String? state, String district) async {
    try {
      final res = await _apiService.getMarkets(state: state, district: district);
      if (mounted && res.data != null && res.data['success'] == true) {
        final List<dynamic> list = res.data['data'] ?? [];
        setState(() {
      ...
      if (mounted && res.data != null && res.data['success'] == true) {
        ...
        if (data != null && data['referencePrice'] != null) {
          setState(() {
            ...
            _marketPriceLocation = data['market'] != null ? '${data['market']}, ${data['district'] ?? ''}' : data['state'];
          });
        }
      }
    } catch (_) {
                ...
                if (snapshot.hasError) {
                  return Center(
                    child: Padding(
                      ...
                      child: Text('Error loading market prices: ${snapshot.error}'),
                    ),
                  );
                }

                final data = snapshot.data?.data;
                final List<dynamic> prices = data != null && data['success'] == true
                    ? (data['data'] ?? [])
                    : [];
                ...
                if (prices.isEmpty) {
                  return const Center(
                    child: Padding(
                      ...
                      child: Text('No historical market price data found for this crop.'),
                    ),
                  );
                }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        title: Text('${item['market'] ?? 'Market'}, ${item['state'] ?? ''}'),
                        subtitle: Text('Variety: ${item['variety'] ?? 'FAQ'} • Date: ${item['date'] ?? ''}'),
                        trailing: Text(
                          '₹${item['modalPrice'] ?? 'N/A'}',
  ...
  Future<void> _submitForm() async {
    ...
    if (_selectedState == null || _selectedState!.isEmpty || _selectedDistrict == null || _selectedDistrict!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    final reqData = {
      ...
      'district': _selectedDistrict,
      'market': _selectedMarket ?? '',
      'location': _selectedMarket != null && _selectedMarket!.isNotEmpty ? _selectedMarket : _selectedDistrict,
      'notes': _notesController.text.trim(),
    };

    if (_isEditing) {
      final updated = await provider.updateRequirement(widget.initialRequirement!.id, reqData);
      ...
      if (mounted) {
        if (updated != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(provider.errorMessage ?? 'Unable to submit your requirement.')),
          );
        }
      }
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_isEditing ? 'Edit Requirement' : 'Create Requirement'),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      initialValue: _commodities.contains(_selectedCommodity) ? _selectedCommodity : null,
                      ...
                      validator: (val) => val == null || val.isEmpty ? 'Please select a crop.' : null,
                    ),
                    ...
                    const SizedBox(height: 6),
                    DropdownButtonFormField<String>(
                      initialValue: _varieties.contains(_selectedVariety) ? _selectedVariety : null,
                      decoration: const InputDecoration(
                        border: OutlineInputBorder(),
                        contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                      ...
                      items: _varieties.isEmpty
                          ? [const DropdownMenuItem(value: 'Standard/Other', child: Text('Standard/Other'))]
                          : _varieties.map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(),
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  const Icon(Icons.trending_up, color: Color(0xFF2563EB), size: 20),
                                  ...
                                  if (_isLoadingMarketPrice)
                                    ...
                                    TextButton(
                                      ...
                                      child: const Text('View Trends', style: TextStyle(fontSize: 12)),
                                    ),
                                ],
                              ),
                              const SizedBox(height: 6),
                              if (_marketReferencePrice != null) ...[
                                ...
                                if (_marketPriceLocation != null || _marketPriceDate != null) ...[
                                  const SizedBox(height: 2),
                                  Text(
                                    [
                                      if (_marketPriceLocation != null) 'Location: $_marketPriceLocation',
                                      if (_marketPriceDate != null) 'Date: $_marketPriceDate',
                                    ].join(' • '),
                    ...
                    TextFormField(
                      ...
                      validator: (val) {
                        if (val == null || val.trim().isEmpty) return 'Please enter your expected price.';
                        final num = double.tryParse(val.trim());
                    ...
                    const SizedBox(height: 6),
                    DropdownButtonFormField<String>(
                      initialValue: _states.contains(_selectedState) ? _selectedState : null,
                      decoration: const InputDecoration(
                        border: OutlineInputBorder(),
                        contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                      ...
                      onChanged: (val) {
                        ...
                        if (val != null) {
                          _loadDistricts(val);
                          if (_selectedCommodity != null) _fetchMarketReferencePrice(_selectedCommodity!);
                        }
                      },
                    ...
                    DropdownButtonFormField<String>(
                      initialValue: _districts.contains(_selectedDistrict) ? _selectedDistrict : null,
                      decoration: const InputDecoration(
                        border: OutlineInputBorder(),
                        contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                    ...
                    DropdownButtonFormField<String>(
                      initialValue: _markets.contains(_selectedMarket) ? _selectedMarket : null,
                      decoration: const InputDecoration(
                        border: OutlineInputBorder(),
                        contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: _isSubmitting
                            ...
                            : Text(
                                _isEditing ? 'Update Requirement' : 'Post Requirement',

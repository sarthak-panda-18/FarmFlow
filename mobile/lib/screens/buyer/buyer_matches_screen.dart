...
class _BuyerMatchesScreenState extends State<BuyerMatchesScreen> {
      ...
      if (res.data != null && res.data['success'] == true) {
        ...
        setState(() {
          _errorMessage = res.data?['message']?.toString() ??
              'Failed to load matching farmer crops';
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  void _openGoogleMaps(MatchModel match) async {
    final success = await _locationService.openGoogleMaps(
      ...
      label: (match.farmerName != null && match.farmerName!.isNotEmpty)
          ? '${match.farmerName} (Farmer Crop)'
          : null,
    );
    if (!success && mounted) {
  ...
  void _showExpressInterestDialog(MatchModel match) {
    final priceController = TextEditingController(
        text: match.buyerExpectedPrice.toStringAsFixed(0));
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(
                  'Available Qty: ${match.farmerAvailableQty} ${match.farmerUnit}'),
              Text(
                  'Farmer Expected: ₹${match.farmerExpectedPrice.toStringAsFixed(0)} / Quintal'),
                ...
                if (match.cropId != null) {
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content:
                            Text('Purchase interest sent directly to Farmer!'),
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const Text('Ranked by Score & Distance',
                      style:
                ...
                Row(
                  ...
                  children: [
                    _InfoColumn(
                      ...
                      value:
                          '${match.farmerAvailableQty.toStringAsFixed(0)} ${match.farmerUnit}',
                      subtext:
                          match.farmerAvailableQty >= match.buyerRequiredQty
                              ? '✓ Full Coverage'
                              : '⚠ Partial',
                      subtextColor:
                          match.farmerAvailableQty >= match.buyerRequiredQty
                  ...
                  child: Row(
                    ...
                    children: [
                      _PriceBox(
                        label: 'Farmer Price',
                        price:
                            '₹${match.farmerExpectedPrice.toStringAsFixed(0)}/Q',
                        color: AppColors.primary,
                      ),
                      _PriceBox(
                        label: 'Market Ref',
                        price:
                            '₹${match.marketReferencePrice.toStringAsFixed(0)}/Q',
                        color: AppColors.textSecondary,
                      ),
                      _PriceBox(
                        label: 'Your Offer',
                        price:
                            '₹${match.buyerExpectedPrice.toStringAsFixed(0)}/Q',
                        color: AppColors.secondary,
                      ),
                    ],
                  ),
